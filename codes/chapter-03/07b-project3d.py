...
def plot_basis_plane(ax, v1, v2, p0, limitx=(0, 4), limity=(0, 4), alpha=0.5, color='g'):
    ...
    # 绘制平面
    ax.plot_surface(xx, yy, zz, alpha=alpha, color=color, label='投影平面')
    ax.plot_wireframe(xx, yy, zz, color='brown', linewidth=0.5, alpha=0.3, rstride=1, cstride=1)


def plot_projected_points(ax, points_3d, points_proj, color_original='r', color_projected='b', line_color='k'):
    """
    ...
    参数:
        ...
        color_original (str): 原始点的颜色，默认为红色 ('r')。
        color_projected (str): 投影点的颜色，默认为蓝色 ('b')。
        line_color (str): 连线的颜色，默认为黑色 ('k')。
    """
    # 绘制原始点
    ax.scatter(points_3d[:, 0], points_3d[:, 1], points_3d[:, 2],
               color=color_original, label='原始数据点', s=100, marker='^')

    # 绘制投影点
    ax.scatter(points_proj[:, 0], points_proj[:, 1], points_proj[:, 2],
               color=color_projected, label='投影数据点', s=100, marker='o')
...
if __name__ == "__main__":
    ...
    # 平面平移向量
    p0 = np.array([0, 0, 0])  # 平面的原点，用于平移平面位置
    ...
    # 绘制原始点和投影点
    plot_projected_points(ax, points_3d, points_proj)
